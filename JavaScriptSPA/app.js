...
//Graph API url for ease of access
var graphAPIPatient = ["https://graph.microsoft.com/v1.0/me/drive/root:/PAS_HighFid_data"];
var grapAPITest = ["https://graph.microsoft.com/v1.0/me/drive/root/search(q='patient-example')?select=name,id,webUrl"];
...
function getPatientData() {
    ...
    if (!user) {
        ...
        var graphCallResponseElement = document.getElementById("graphResponse");
        graphCallResponseElement.parentElement.classList.remove("hidden");
        //graphCallResponseElement.innerHTML = JSON.stringify(user, null, 4);

        //var jsonDataElement = document.getElementById("patientJson");
        ...
        userAgentApplication.acquireTokenSilent(graphAPIScopes)
            .then(function (token) {
                //After the access token is acquired, call the Web API, sending the acquired token
                callWebApiWithToken(grapAPITest, token, graphCallResponseElement, null);
